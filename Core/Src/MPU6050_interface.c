...
void MPU6050_Read_Accel(struct MPU6050_Data* data) {
    uint8_t buffer[6];
    HAL_I2C_Mem_Read_DMA(&hi2c1, MPU6050_ADDR, ACCEL_XOUT_H_REG, I2C_MEMADD_SIZE_8BIT, buffer, sizeof(buffer),HAL_MAX_DELAY);
...
void MPU6050_Read_Gyro(struct MPU6050_Data* data) {
    uint8_t buffer[6];
    HAL_I2C_Mem_Read_DMA(&hi2c1, MPU6050_ADDR, GYRO_XOUT_H_REG, I2C_MEMADD_SIZE_8BIT, buffer, sizeof(buffer),HAL_MAX_DELAY);
